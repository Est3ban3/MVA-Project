...
def main():
    ...
    for channel in CHANNELS:
        ...
        for model in MODELS:
            ...
            # supervisor's MultiFit steps (mnwd / mfp / ml) merged into one
            # invocation, same reason as the per-model fits above. `n` and
            # `d`/`p` are what produce the combined pre/post-fit plots —
            # plain `mwfl` writes the limit but none of them.
            combine_cmds.append(f"trex-fitter mnwdfpl configs/{name}")
            # separated variant for run_fits.sh, at the supervisor's own
            # granularity (mnwd / mfp / ml)
            simple_combine_cmds += [
                f"trex-fitter {steps} configs/{name}"
                for steps in ("mnwd", "mfp", "ml")
            ] + [""]
    n_fits = len(fit_cmds)
    script = "\n".join(
        [
